...
export function BalanceClassic() {
  ...
  const rows = useMemo(() => {
    ...
    if (txnFilter !== 'All') r = r.filter(s => s.action === txnFilter)
  ...
  const { displayRows, summary } = useMemo(() => {
    ...
    const out: Array<{ idx: number; tx: StockTransaction; netValue: number }> = []
    rows.forEach((tx, i) => {
      let net: number
      if (tx.action === 'Buy') {
        buyQty += tx.quantity
      ...
      out.push({ idx: i + 1, tx, netValue: net })
    })
    return {
      displayRows: out,
      summary: { buyQty, sellQty, buyTotal, sellTotal, brokerage, sellMinusBuy: sellTotal - buyTotal },
    }
  }, [rows])
